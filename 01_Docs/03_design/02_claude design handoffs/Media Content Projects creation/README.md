...
## Overview
Almanach is a desktop news-aggregator UI. A left sidebar organizes RSS-style **sources** into a drag-and-drop tree of **groups → subgroups**. The main pane shows a filterable **article feed**. Two features are the focus of this handoff:
...
- **Popover dismissal**: outside mousedown or Escape.
- **Animations**: popover 0.12s ease fade/translate; most hover/color transitions 0.12s.

## State Management
Recreate this state (in the prototype it lives in `app.jsx`):
- `state` — `{ groups, ungroupedSourceIds }`. `groups[]` = `{ id, name, collapsed, sourceIds[], subgroups[] }`; `subgroups[]` = `{ id, name, collapsed, sourceIds[] }`.
...
Handlers to port: `addProject`, `renameProject`, `deleteProject`, `toggleArticleInProject(projectId, articleId)`, `createProjectWithArticle(name, articleId)`.
...
- `Almanach.html` — entry point; loads React/Babel and the scripts in order: `data.js`, `tweaks-panel.jsx`, `sidebar.jsx`, `projects.jsx`, `filter-bar.jsx`, `feed.jsx`, `app.jsx`. Contains the `#root` mount and the `#__om-edit-overrides` style block.
- `app.jsx` — root component, all app state, localStorage persistence, project handlers, Tweaks panel.
...
- `styles.css` — all styling and design tokens (`:root`). The Projects/bookmark/save-menu/project-view styles are at the end of the file.
